...
public class Sphere_S1_Script : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        disty = 0;
        distz = 0;
        //go1.GetComponent<Rigidbody>().velocity = new Vector3(Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f), Random.Range(-1.0f, 1.0f));
        go1.GetComponent<Rigidbody>().velocity = new Vector3(0f, 1f, 3f);     
        v_Vector = go1.GetComponent<Rigidbody>().velocity;
        x_Vector = go1.GetComponent<Rigidbody>().transform.localPosition;
        go2 = GameObject.Find("Particle2");
    ...
    private void FixedUpdate() {
        ...
        //Angles of Fx, Fy and Fz forces after decomposing the pulling F force
        angleV = go2.GetComponent<Transform>().localPosition - x_Vector;
        angleV = go1.transform.InverseTransformDirection(angleV);
        ...
        if (distobjx != 0){
            a.x = (1/(distobjx))*0.01f /go1.GetComponent<Rigidbody>().mass;

        }
        //Acceleration on Y-axis
        if (distobjy != 0){
            a.y = (1/(distobjy))*0.01f/go1.GetComponent<Rigidbody>().mass;

        }
        //Acceleration on Z-axis
        if (distobjz != 0){
            a.z = (1/(distobjz))*0.01f /go1.GetComponent<Rigidbody>().mass;
        ...
        Vector3 distTemp = go1.GetComponent<Transform>().localPosition - sphere.GetComponent<Transform>().localPosition;
        if (distTemp.z < 0.4 && distTemp.y < 0.4 && distTemp.x < 0.4 && distTemp.x > -0.4 &&  distTemp.y > -0.4 && distTemp.z > -0.4) {       
                ...
                go1.GetComponent<Rigidbody>().velocity = v_next_Vector;
                v_Vector = go1.GetComponent<Rigidbody>().velocity;
                ...
                {
                    go1.GetComponent<Rigidbody>().transform.localPosition = x_next_Vector;
